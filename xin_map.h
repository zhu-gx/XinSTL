...
//
// 异常保证：
// XinSTL::map<Key, T> / XinSTL::multimap<Key, T> 满足基本异常保证，对以下等函数做强异常安全保证：
//   * emplace
//   * emplace_hint
//   * insert
...
// 模板类 map，键值不允许重复
// 参数一代表键值类型，参数二代表实值类型，参数三代表键值的比较方式，缺省使用 XinSTL::less
template <class Key, class T, class Compare = XinSTL::less<Key>>
class map
{
public:
  // map 的嵌套型别定义
  ...
  typedef XinSTL::pair<const Key, T>  value_type;
...
private:
  // 以 XinSTL::rb_tree 作为底层机制
  typedef XinSTL::rb_tree<value_type, key_compare>  base_type;
  base_type tree_;
...
public:
  ...
  mapped_type& operator[](key_type&& key)
  {
    ...
    // it->first >= key
    if (it == end() || key_comp()(key, it->first))
      it = emplace_hint(it, XinSTL::move(key), T{});
    return it->second;
  }
  ...
  {
    return tree_.emplace_unique(XinSTL::forward<Args>(args)...);
  }

  template <class ...Args>
  iterator emplace_hint(iterator hint, Args&& ...args)
  {
    return tree_.emplace_unique_use_hint(hint, XinSTL::forward<Args>(args)...);
  }
  ...
  {
    return tree_.insert_unique(XinSTL::move(value));
  }
  ...
  {
    return tree_.insert_unique(hint, XinSTL::move(value));
  }
...
public:
  ...
  {
    return tree_.emplace_multi(XinSTL::forward<Args>(args)...);
  }

  template <class ...Args>
  iterator emplace_hint(iterator hint, Args&& ...args)
  {
    return tree_.emplace_multi_use_hint(hint, XinSTL::forward<Args>(args)...);
  }
  ...
  {
    return tree_.insert_multi(XinSTL::move(value));
  }
  ...
  iterator insert(iterator hint, value_type&& value)
  {
    return tree_.insert_multi(hint, XinSTL::move(value));
  }
