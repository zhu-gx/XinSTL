...
namespace XinSTL{
    ...
    public:
        ...
        void emplace_front(Args&& ...args){
            ...
            auto link_node = creat_node(XinSTL::forward<Args>(args)...);
        ...
        iterator emplace(const_iterator pos,Args&& ...args){
            THROW_LENGTH_ERROR_IF(size_ > max_size()-1,"list<T>'s size too big");
            auto link_node = creat_node(XinSTL::forward<Args>(args)...);
            link_node(pos.node_,link_node->as_base(),link_node->as_base());
            size_++;
        ...
        iterator insert(const_iterator pos,const value_type& value){
            THROW_LENGTH_ERROR_IF(size_ > max_size()-1,"list<T>'s size too big");
            auto link_node = creat_node(value);
            size_++;
        ...
        iterator insert(const_iterator pos,value_type&& value){
            THROW_LENGTH_ERROR_IF(size_ > max_size()-1,"list<T>'s size too big");
            auto link_node = create_node(XinSTL::move(value));
            size_++;
            return link_iter_node(pos,link_node->as_base());
        }
        ...
        template<class Iter,typename std::enable_if<XinSTL::is_input_iterator<Iter>::value,int>::type = 0>
        ...
        void sort(){
            list_sort(begin(),end(),size(),XinSTL::less<T>());
        }
    ...
    private:
        ...
        void link_nodes_at_front(base_ptr frist,base_ptr last);
        void link_nodes_at_back(base_ptr frist,base_ptr last);
...
void list<T>::reverse()
{
  ...
  {
    XinSTL::swap(i.node_->prev, i.node_->next);
    i.node_ = i.node_->prev;
  }
  XinSTL::swap(e.node_->prev, e.node_->next);
}
...
list<T>::create_node(Args&& ...args)
{
  ...
  try
  {
    data_allocator::construct(XinSTL::address_of(p->value), XinSTL::forward<Args>(args)...);
    p->prev = nullptr;
    p->next = nullptr;
  }
  catch (...)
...
{
  data_allocator::destroy(XinSTL::address_of(p->value));
  node_allocator::deallocate(p);
}
...
bool operator<(const list<T>& lhs, const list<T>& rhs)
{
  return XinSTL::lexicographical_compare(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend());
}
