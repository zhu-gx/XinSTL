...
struct char_traits
{
  ...
  {
    XINSTL_DEBUG(src + n <= dst || dst + n <= src);
    char_type* r = dst;
    for (; n != 0; --n, ++dst, ++src)
      *dst = *src;
    return r;
...
// 模板类 basic_string
// 参数一代表字符类型，参数二代表萃取字符类型的方式，缺省使用 XinSTL::char_traits
template <class CharType, class CharTraits = XinSTL::char_traits<CharType>>
class basic_string
{
public:
  ...
  typedef XinSTL::allocator<CharType>               allocator_type;
  typedef XinSTL::allocator<CharType>               data_allocator;
...
public:
  ...
  {
    count = XinSTL::min(count, size_ - index);
  ...
  {
    XINSTL_DEBUG(begin() <= first && last <= end() && first <= last);
...
private:
  ...
  template <class Iter>
  void          copy_init(Iter first, Iter last, XinSTL::input_iterator_tag);
  template <class Iter>
  void          copy_init(Iter first, Iter last, XinSTL::forward_iterator_tag);
...
insert(const_iterator pos, Iter first, Iter last)
{
  ...
  if (pos == end())
  {
    XinSTL::uninitialized_copy(first, last, end());
    size_ += len;
    return r;
  }
  char_traits::move(r + len, r, len);
  XinSTL::uninitialized_copy(first, last, r);
  size_ += len;
  return r;
}
...
{
  auto n1 = XinSTL::min(count1, size_ - pos1);
...
{
  auto n1 = XinSTL::min(count1, size_ - pos1);
  auto n2 = XinSTL::min(count2, other.size_ - pos2);
...
swap(basic_string& rhs) noexcept
{
  if (this != &rhs)
  {
    XinSTL::swap(buffer_, rhs.buffer_);
...
{
  const auto init_size = XinSTL::max(static_cast<size_type>(STRING_INIT_SIZE), n + 1);
...
template <class Iter>
void basic_string<CharType, CharTraits>::
copy_init(Iter first, Iter last, XinSTL::input_iterator_tag)
{
...
template <class Iter>
void basic_string<CharType, CharTraits>::
copy_init(Iter first, Iter last, XinSTL::forward_iterator_tag)
{
  ...
  try
  {
    ...
    size_ = n;
    cap_ = init_size;
    XinSTL::uninitialized_copy(first, last, buffer_);
  }
  catch (...)
  {
    buffer_ = nullptr;
...
{
  const auto init_size = XinSTL::max(static_cast<size_type>(STRING_INIT_SIZE), count + 1);
  buffer_ = data_allocator::allocate(init_size);
...
append_range(Iter first, Iter last)
{
  ...
  XinSTL::uninitialized_copy_n(first, n, buffer_ + size_);
  size_ += n;
  return *this;
}
...
{
  const auto new_cap = XinSTL::max(cap_ + need, cap_ + (cap_ >> 1));
...
reallocate_and_fill(iterator pos, size_type n, value_type ch)
{
  ...
  const auto new_cap = XinSTL::max(old_cap + n, old_cap + (old_cap >> 1));
  auto new_buffer = data_allocator::allocate(new_cap);
...
reallocate_and_copy(iterator pos, const_iterator first, const_iterator last)
{
  ...
  auto new_buffer = data_allocator::allocate(new_cap);
  auto e1 = char_traits::move(new_buffer, buffer_, r) + r;
  auto e2 = XinSTL::uninitialized_copy_n(first, n, e1) + n;
  char_traits::move(e2, buffer_ + r, size_ - r);
  data_allocator::deallocate(buffer_, old_cap);
  buffer_ = new_buffer;
  size_ += n;
...
operator+(basic_string<CharType, CharTraits>&& lhs,
          const basic_string<CharType, CharTraits>& rhs)
{
  basic_string<CharType, CharTraits> tmp(XinSTL::move(lhs));
  tmp.append(rhs);
  return tmp;
}
...
{
  basic_string<CharType, CharTraits> tmp(XinSTL::move(rhs));
