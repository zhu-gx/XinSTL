...
// 模板类 queue
// 参数一代表数据类型，参数二代表底层容器类型，缺省使用 XinSTL::deque 作为底层容器
//queue是一种先进先出的数据结构
//封闭deque的底端出口和前端入口获得
//没有迭代器
template <class T, class Container = XinSTL::deque<T>>
class queue
{
public:
...
public:
  ...
  { c_.emplace_back(XinSTL::forward<Args>(args)...); }

  void push(const value_type& value) 
  { c_.push_back(value); }
  void push(value_type&& value)      
  { c_.emplace_back(XinSTL::move(value)); }
  ...
  void swap(queue& rhs) noexcept(noexcept(XinSTL::swap(c_, rhs.c_)))
  { XinSTL::swap(c_, rhs.c_); }
...
// 模板类 priority_queue
// 参数一代表数据类型，参数二代表容器类型，缺省使用 XinSTL::vector 作为底层容器
// 参数三代表比较权值的方式，缺省使用 XinSTL::less 作为比较方式
template <class T, class Container = XinSTL::vector<T>,
  class Compare = XinSTL::less<typename Container::value_type>>
...
public:
  ...
  explicit priority_queue(size_type n)
    :c_(n)
  {
    XinSTL::make_heap(c_.begin(), c_.end(), comp_);
  }
  ...
  priority_queue(priority_queue&& rhs) 
    :c_(XinSTL::move(rhs.c_)), comp_(rhs.comp_)
  {
    XinSTL::make_heap(c_.begin(), c_.end(), comp_);
  }
...
public:
  ...
  {
    c_.emplace_back(XinSTL::forward<Args>(args)...);
    XinSTL::push_heap(c_.begin(), c_.end(), comp_);
  }
  ...
  void push(value_type&& value)
  {
    c_.push_back(XinSTL::move(value));
    XinSTL::push_heap(c_.begin(), c_.end(), comp_);
  }

  void pop()
  {
    XinSTL::pop_heap(c_.begin(), c_.end(), comp_);
    c_.pop_back();
  }
  ...
  void swap(priority_queue& rhs) noexcept(noexcept(XinSTL::swap(c_, rhs.c_)) &&
                                          noexcept(XinSTL::swap(comp_, rhs.comp_)))
  {
    XinSTL::swap(c_, rhs.c_);
    XinSTL::swap(comp_, rhs.comp_);
  }
