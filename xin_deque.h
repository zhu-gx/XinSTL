...
//
// 异常保证：
// XinSTL::deque<T> 满足基本异常保证，部分函数无异常保证，并对以下等函数做强异常安全保证：
//   * emplace_front
//   * emplace_back
//   * emplace
//   * push_front
...
public:
  // deque 的型别定义
  typedef XinSTL::allocator<T>                      allocator_type;
  typedef XinSTL::allocator<T>                      data_allocator;
  typedef XinSTL::allocator<T*>                     map_allocator;
  ...
  typedef XinSTL::reverse_iterator<iterator>        reverse_iterator;
  typedef XinSTL::reverse_iterator<const_iterator>  const_reverse_iterator;
...
public:
  ...
  template <class IIter, typename std::enable_if<
    XinSTL::is_input_iterator<IIter>::value, int>::type = 0>
  deque(IIter first, IIter last)
  { copy_init(first, last, iterator_category(first)); }

  deque(std::initializer_list<value_type> ilist)
  {
    copy_init(ilist.begin(), ilist.end(), XinSTL::forward_iterator_tag());
  }

  deque(const deque& rhs)
  {
    copy_init(rhs.begin(), rhs.end(), XinSTL::forward_iterator_tag());
  }
  deque(deque&& rhs) noexcept
    :begin_(XinSTL::move(rhs.begin_)),
    end_(XinSTL::move(rhs.end_)),
    map_(rhs.map_),
...
public:
  ...
  { copy_assign(ilist.begin(), ilist.end(), XinSTL::forward_iterator_tag{}); }
  ...
  void     push_front(value_type&& value) { emplace_front(XinSTL::move(value)); }
  void     push_back(value_type&& value)  { emplace_back(XinSTL::move(value)); }
...
deque<T>& deque<T>::operator=(const deque& rhs)
{
  if (this != &rhs)
  {
    ...
    if (len >= rhs.size())
    {
      erase(XinSTL::copy(rhs.begin_, rhs.end_, begin_), end_);
    }
    else
    {
      iterator mid = rhs.begin() + static_cast<difference_type>(len);
      XinSTL::copy(rhs.begin_, mid, begin_);
...
void deque<T>::emplace_front(Args&& ...args)
{
  ...
  {
    data_allocator::construct(begin_.cur - 1, XinSTL::forward<Args>(args)...);
    --begin_.cur;
  }
  else
  {
    ...
    {
      --begin_;
      data_allocator::construct(begin_.cur, XinSTL::forward<Args>(args)...);
    }
    catch (...)
    {
      ++begin_;
...
void deque<T>::emplace_back(Args&& ...args)
{
  ...
  {
    data_allocator::construct(end_.cur, XinSTL::forward<Args>(args)...);
    ++end_.cur;
  }
  else
  {
...
typename deque<T>::iterator deque<T>::emplace(iterator pos, Args&& ...args)
{
  ...
  {
    emplace_front(XinSTL::forward<Args>(args)...);
    return begin_;
  }
  else if (pos.cur == end_.cur)
  {
    emplace_back(XinSTL::forward<Args>(args)...);
    return end_ - 1;
  }
  return insert_aux(pos, XinSTL::forward<Args>(args)...);
}
...
deque<T>::insert(iterator position, value_type&& value)
{
  if (position.cur == begin_.cur)
  {
    emplace_front(XinSTL::move(value));
    return begin_;
  }
  else if (position.cur == end_.cur)
  {
  ...
  {
    return insert_aux(position, XinSTL::move(value));
  }
}
...
void deque<T>::insert(iterator position, size_type n, const value_type& value)
{
  if (position.cur == begin_.cur)
  {
    ...
    XinSTL::uninitialized_fill_n(new_begin, n, value);
    begin_ = new_begin;
  }
  else if (position.cur == end_.cur)
  {
    ...
    auto new_end = end_ + n;
    XinSTL::uninitialized_fill_n(end_, n, value);
    end_ = new_end;
  }
  else
  {
...
deque<T>::erase(iterator position)
{
  ...
  {
    XinSTL::copy_backward(begin_, position, next);
    pop_front();
  }
  else
  {
    XinSTL::copy(next, end_, position);
    pop_back();
  }
...
deque<T>::erase(iterator first, iterator last)
{
  ...
  else
  {
    ...
    {
      XinSTL::copy_backward(begin_, first, last);
...
void deque<T>::clear()
{
  ...
  { // 有两个以上的缓冲区
    XinSTL::destroy(begin_.cur, begin_.last);
    XinSTL::destroy(end_.first, end_.cur);
  }
  else
  {
    XinSTL::destroy(begin_.cur, end_.cur);
  }
  shrink_to_fit();
  end_ = begin_;
}
...
void deque<T>::swap(deque& rhs) noexcept
{
  if (this != &rhs)
  {
    ...
    XinSTL::swap(map_, rhs.map_);
    XinSTL::swap(map_size_, rhs.map_size_);
  }
}
...
map_init(size_type nElem)
{
  ...
  map_size_ = XinSTL::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), nNode + 2);
  try
  {
...
fill_init(size_type n, const value_type& value)
{
  ...
  if (n != 0)
  {
    ...
    {
      XinSTL::uninitialized_fill(*cur, *cur + buffer_size, value);
    }
    XinSTL::uninitialized_fill(end_.first, end_.cur, value);
  }
}
...
{
  const size_type n = XinSTL::distance(first, last);
  map_init(n);
...
copy_init(FIter first, FIter last, forward_iterator_tag)
{
  ...
  {
    auto next = first;
    XinSTL::advance(next, buffer_size);
    XinSTL::uninitialized_copy(first, next, *cur);
    first = next;
  }
  XinSTL::uninitialized_copy(first, last, end_.first);
}
...
{  
  ...
  const size_type len2 = XinSTL::distance(first, last);
  if (len1 < len2)
  {
    auto next = first;
    XinSTL::advance(next, len1);
    XinSTL::copy(first, next, begin_);
    insert_dispatch(end_, next, last, forward_iterator_tag{});
  }
  else
  {
    erase(XinSTL::copy(first, last, begin_), end_);
  }
}
...
{
  const size_type elems_before = position - begin_;
  value_type value_copy = value_type(XinSTL::forward<Args>(args)...);
  if (elems_before < (size() / 2))
  { // 在前半段插入
    emplace_front(front());
    auto front1 = begin_;
  ...
  { // 在后半段插入
    ...
    --back2;
    position = begin_ + elems_before;
    XinSTL::copy_backward(position, back2, back1);
  }
  *position = XinSTL::move(value_copy);
  return position;
}
...
fill_insert(iterator position, size_type n, const value_type& value)
{
  ...
  if (elems_before < (len / 2))
  {
    ...
    try
    {
      if (elems_before >= n)
      {
        ...
        XinSTL::uninitialized_copy(begin_, begin_n, new_begin);
        begin_ = new_begin;
        XinSTL::copy(begin_n, position, old_begin);
        XinSTL::fill(position - n, position, value_copy);
      }
      else
      {
        XinSTL::uninitialized_fill(
          XinSTL::uninitialized_copy(begin_, position, new_begin), begin_, value_copy);
        begin_ = new_begin;
        XinSTL::fill(old_begin, position, value_copy);
      }
    }
    catch (...)
    {
  ...
  else
  {
    ...
    try
    {
      ...
      {
        auto end_n = end_ - n;
        XinSTL::uninitialized_copy(end_n, end_, end_);
        end_ = new_end;
        XinSTL::copy_backward(position, end_n, old_end);
        XinSTL::fill(position, position + n, value_copy);
      }
      else
      {
        XinSTL::uninitialized_fill(end_, position + n, value_copy);
        XinSTL::uninitialized_copy(position, end_, position + n);
        end_ = new_end;
        XinSTL::fill(position, old_end, value_copy);
      }
    }
    catch (...)
    {
  ...
  if (elems_before < (len / 2))
  {
    ...
    try
    {
      if (elems_before >= n)
      {
        ...
        begin_ = new_begin;
        XinSTL::copy(begin_n, position, old_begin);
        XinSTL::copy(first, last, position - n);
      }
      else
      {
        auto mid = first;
        XinSTL::advance(mid, n - elems_before);
        XinSTL::uninitialized_copy(first, mid,
                                  XinSTL::uninitialized_copy(begin_, position, new_begin));
        begin_ = new_begin;
        XinSTL::copy(mid, last, old_begin);
      }
    }
    catch (...)
    {
  ...
  else
  {
    ...
    try
    {
      if (elems_after > n)
      {
        ...
        end_ = new_end;
        XinSTL::copy_backward(position, end_n, old_end);
        XinSTL::copy(first, last, position);
      }
      else
      {
        auto mid = first;
        XinSTL::advance(mid, elems_after);
        XinSTL::uninitialized_copy(position, end_,
                                  XinSTL::uninitialized_copy(mid, last, end_));
        end_ = new_end;
        XinSTL::copy(first, mid, position);
      }
    }
    catch (...)
    {
...
insert_dispatch(iterator position, FIter first, FIter last, forward_iterator_tag)
{
  ...
  if (position.cur == begin_.cur)
  {
    ...
    try
    {
      XinSTL::uninitialized_copy(first, last, new_begin);
      begin_ = new_begin;
    }
    catch (...)
    {
  ...
  else if (position.cur == end_.cur)
  {
    ...
    try
    {
      XinSTL::uninitialized_copy(first, last, end_);
      end_ = new_end;
    }
    catch (...)
    {
...
{
  const size_type new_map_size = XinSTL::max(map_size_ << 1,
...
bool operator<(const deque<T>& lhs, const deque<T>& rhs)
{
  return XinSTL::lexicographical_compare(
