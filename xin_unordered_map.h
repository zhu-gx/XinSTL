...
//
// 异常保证：
// XinSTL::unordered_map<Key, T> / XinSTL::unordered_multimap<Key, T> 满足基本异常保证，对以下等函数做强异常安全保证：
//   * emplace
//   * emplace_hint
//   * insert

#include<xin_hashtable.h>

namespace XinSTL
{

// 模板类 unordered_map，键值不允许重复
// 参数一代表键值类型，参数二代表实值类型，参数三代表哈希函数，缺省使用 XinSTL::hash
// 参数四代表键值比较方式，缺省使用 XinSTL::equal_to
template <class Key, class T, class Hash = XinSTL::hash<Key>, class KeyEqual = XinSTL::equal_to<Key>>
class unordered_map
{
private:
  // 使用 hashtable 作为底层机制
  typedef hashtable<XinSTL::pair<const Key, T>, Hash, KeyEqual> base_type;
  base_type ht_;
...
public:
  ...
  unordered_map(InputIterator first, InputIterator last,
                ...
                const Hash& hash = Hash(),
                const KeyEqual& equal = KeyEqual())
    : ht_(XinSTL::max(bucket_count, static_cast<size_type>(XinSTL::distance(first, last))), hash, equal)
  {
  ...
  unordered_map(std::initializer_list<value_type> ilist,
                ...
                const Hash& hash = Hash(),
                const KeyEqual& equal = KeyEqual())
    :ht_(XinSTL::max(bucket_count, static_cast<size_type>(ilist.size())), hash, equal)
  {
  ...
  { return ht_.emplace_unique(XinSTL::forward<Args>(args)...); }

  template <class ...Args>
  iterator emplace_hint(const_iterator hint, Args&& ...args)
  { return ht_.emplace_unique_use_hint(hint, XinSTL::forward<Args>(args)...); }
  ...
  { return ht_.emplace_unique(XinSTL::move(value)); }

  iterator insert(const_iterator hint, const value_type& value)
  { return ht_.insert_unique_use_hint(hint, value); }
  iterator insert(const_iterator hint, value_type&& value)
  { return ht_.emplace_unique_use_hint(hint, XinSTL::move(value)); }
  ...
  mapped_type& operator[](key_type&& key)
  {
    iterator it = ht_.find(key);
    if (it.node == nullptr)
      it = ht_.emplace_unique(XinSTL::move(key), T{}).first;
    return it->second;
  }
...
public:
  ...
  { return ht_.emplace_multi(XinSTL::forward<Args>(args)...); }

  template <class ...Args>
  iterator emplace_hint(const_iterator hint, Args&& ...args)
  { return ht_.emplace_multi_use_hint(hint, XinSTL::forward<Args>(args)...); }
  ...
  { return ht_.emplace_multi(XinSTL::move(value)); }

  iterator insert(const_iterator hint, const value_type& value)
  { return ht_.insert_multi_use_hint(hint, value); }
  iterator insert(const_iterator hint, value_type&& value)
  { return ht_.emplace_multi_use_hint(hint, XinSTL::move(value)); }
